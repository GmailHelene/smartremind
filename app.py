...
def init_db():
    ...
    try:
        ...
        if conn:
            ...
            # Create reminders table with datetime column
            cur.execute("""
...
def check_reminders_for_notifications():
    ...
    with app.app_context():
        try:
            # Logg minnebruk før sjekk
            process = psutil.Process()
            memory_before = process.memory_info().rss / 1024 / 1024
            logger.info(f"Minnebruk før påminnelsessjekk: {memory_before:.2f} MB")
    ...
    if conn:
        try:
            ...
            cur.execute("""
                ...
                VALUES (%s, 'standard', '{"notifications": true, "daily_goal": 5}')
            """, (user_id,))
            conn.commit()
            cur.close()
        except Exception as e:
